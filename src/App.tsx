    ...
    return (
      <div className="grid min-h-screen place-items-center bg-slate-50 dark:bg-slate-800 px-4 text-center">
        <div className="max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8 shadow-sm">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500 dark:text-slate-300">The property database isn't configured for this environment. If you're the site owner, add the <span className="font-mono text-xs font-bold text-slate-700 dark:text-slate-200">VITE_SUPABASE_URL</span> and <span className="font-mono text-xs font-bold text-slate-700 dark:text-slate-200">VITE_SUPABASE_ANON_KEY</span> environment variables in your hosting dashboard and redeploy.</p>
        </div>
      </div>
    );
  }
...
function Header() {
  ...
  const ThemeToggleButton = ({ className }: { className?: string }) => (
    <button
      ...
      className={`grid h-10 w-10 shrink-0 place-items-center rounded-full border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-300 transition hover:border-forest-300 hover:bg-forest-50 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-600 dark:hover:bg-slate-800 ${className ?? ''}`}
    >
          ...
          {user ? (
            <div className="relative">
              <button onClick={() => setAccountOpen(!accountOpen)} className="flex items-center gap-2 rounded-full border border-slate-200 py-1.5 pl-1.5 pr-3 transition hover:border-forest-300 dark:border-slate-700 dark:hover:border-slate-600">
                ...
                <ChevronDown size={15} className="text-slate-400 dark:text-slate-400" />
              </button>
              {accountOpen && (
                <>
      ...
      {open && (
        <nav className="border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 pb-5 pt-3 md:hidden">
          <div className="flex flex-col gap-1">
            {navLinks.map((route) => <NavLink key={route.to} to={route.to} end={route.to === '/'} onClick={() => setOpen(false)} className={({ isActive }) => `rounded-lg px-3 py-3 text-sm font-semibold ${isActive ? 'bg-forest-50 text-forest-800 dark:text-forest-300 dark:bg-slate-800 dark:text-white' : 'text-slate-600 dark:text-slate-200 dark:text-slate-300'}`}>{route.label}</NavLink>)}
            {user ? (
              <>
                <NavLink to="/account" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-sm font-semibold text-slate-600 dark:text-slate-200 dark:text-slate-300">My profile</NavLink>
                <NavLink to="/saved" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 text-sm font-semibold text-slate-600 dark:text-slate-200 dark:text-slate-300">Saved listings</NavLink>
...
function NavItem({ to, children }: { to: string; children: ReactNode }) {
  return <NavLink to={to} end={to === '/'} className={({ isActive }) => `text-[13px] font-bold transition ${isActive ? 'text-forest-800 dark:text-forest-300' : 'text-slate-500 dark:text-slate-300 hover:text-forest-800'}`}>{children}</NavLink>;
}
...
function ImageUploader({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  return (
    <div className="sm:col-span-2">
      <label className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-400">Photo</label>
