    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-3 sm:flex">
          ...
          <Link to="/admin" className="flex items-center gap-2 rounded-full bg-forest-800 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-forest-900/10 transition hover:-translate-y-0.5 hover:bg-forest-700">List Your Property <ArrowRight size={16} /></Link>
        </div>
...
function Footer() { return <footer className="bg-forest-950 text-white"><div className="mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 md:grid-cols-[1.4fr_1fr_1fr_1.1fr] lg:px-8"><div><Link to="/" className="inline-flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white/10 text-gold-300"><HomeIcon size={20} /></span><span className="leading-none"><span className="block text-[15px] font-extrabold tracking-[0.2em]">PACHU</span><span className="mt-1 block text-[10px] font-semibold tracking-[0.38em] text-gold-300">HAVENS</span></span></Link><p className="mt-6 max-w-xs text-sm leading-6 text-emerald-100/60">A considered way to find your next home, plot, or place to build in Kenya.</p><div className="mt-6 flex gap-2"><SocialIcon icon={<Instagram size={16} />} /><SocialIcon icon={<Facebook size={16} />} /><SocialIcon icon={<Twitter size={16} />} /></div></div><FooterColumn title="Explore" links={[['Properties', '/properties'], ['Land & Plots', '/land-plots'], ['About Us', '/about'], ['Saved listings', '/saved']]} /><FooterColumn title="For owners" links={[['List your property', '/admin'], ['Admin dashboard', '/admin']]} /><div><h3 className="text-sm font-bold text-gold-300">Get in touch</h3><div className="mt-5 space-y-4 text-sm text-emerald-100/70"><p className="flex items-center gap-3"><Phone size={15} /> +254 700 000 000</p><p className="flex items-center gap-3"><Mail size={15} /> hello@pachuhavens.co.ke</p><p className="flex items-center gap-3"><MapPin size={15} /> Nairobi, Kenya</p></div></div></div><div className="border-t border-white/10 px-4 py-5 text-center text-xs text-emerald-100/40 sm:px-6">© 2024 Pachu Havens. Spaces with a little more soul.</div></footer>; }
