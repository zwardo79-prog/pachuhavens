    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to="/" className="group flex items-center gap-3" onClick={() => setOpen(false)}>
          ...
          <span className="leading-none"><span className="block text-[14px] font-extrabold tracking-[0.15em] text-forest-900">PACHU HAVEN</span><span className="mt-1 block text-[10px] font-semibold tracking-[0.3em] text-gold-600">HOMES LTD</span></span>
        </Link>
...
function BrowsePage({ title, eyebrow, categories: pageCategories, properties, loading }: { title: string; eyebrow: string; categories: PropertyCategory[]; properties: Property[]; loading: boolean }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20"><div className="max-w-2xl"><p className="eyebrow">{eyebrow}</p><h1 className="mt-3 text-4xl font-extrabold tracking-tight text-forest-950 sm:text-5xl">{title}</h1><p className="mt-5 text-lg leading-8 text-slate-500">Explore a considered collection of spaces across Kenya, with local guidance at every step.</p></div><div className="my-10 flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4"><label className="block min-w-[200px] flex-1 text-xs font-bold uppercase tracking-wider text-slate-400">Location<div className="mt-2 flex items-center gap-2"><MapPin size={15} className="text-gold-600" /><input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Nairobi, Nakuru, Naivasha..." className="w-full bg-transparent text-sm font-semibold text-slate-800 outline-none placeholder:text-slate-400" /></div></label><SelectField label="Category" value={category} options={['All types', ...pageCategories]} onChange={setCategory} /><SelectField label="Status" value={status} options={['All statuses', ...statuses]} onChange={setStatus} /></div>{loading ? <LoadingGrid /> : filtered.length ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map((property) => <PropertyCardWithHeart key={property.id} property={property} />)}</div> : <EmptyState />}</main>;
}
